...
def updatepoints(points):
    ...
    differences = coords.reshape(1000,1,2)-coords ## gets the x and y difference between every particle from every other particle
    i = np.arange(1000)
    ...
    neighbourdist = (coords - coords[neighbours]) ## gets the x and y difference of the rticke and the nearest psrticle to it
    neighbourdist = (neighbourdist**2).sum(1) ## gets the euclidian distance between a particle and its nearest neighbour
    points[:,4] = np.where(neighbourdist[:]<100,1,0) ## checks if particle is close enough to its nearest neighbour to be colliding essentially checking is distance is less than or equal to double the radius of the particle
    return points
